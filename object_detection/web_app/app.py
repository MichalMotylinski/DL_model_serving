...
def detect(frame, stub, model_name="uk_birds"):
    """
    ...
    for score in output_dict["detection_scores"]:
        if score > threshold:
            ...
            detections.append([categories[output_dict["detection_classes"][idx]]["name"],
                               ...
                               viz_utils.STANDARD_COLORS[
                                   output_dict["detection_classes"][idx] % len(viz_utils.STANDARD_COLORS)],
                               output_dict["detection_boxes"][idx][0]])
        else:
            break

    """
    Reorder items to be displayed in a table. 
    After sorting the classes in a table are displayed to mimic the order of boxes on image.
    The order of detection boxes goes from top right to bottom left.
    """
    detections = sorted(detections, key=operator.itemgetter(3))

    # Remove detection values as they will not be displayed on the web page.
    for i in detections:
        i.pop(-1)

    # Remove blue tint from the results image
    image_np = image_np[:, :, [2, 1, 0]]

    """
    Important note about the box drawing settings:
        - Drawing maximum of 50 boxes because it is unlikely that a single image will contain more objects.
          Moreover this will reduce amount of data being created.
        - Class names are not displayed as they often take to much space on the picture.
          Instead little boxes are displayed in the table with results showing the color matching detection class.
    """
...
def results(filename):
    ...
    im = Image.fromarray(image_np_inf)
    ...
    return render_template("results.html", general_style=general_style, page_style=page_style,
                           ...
                           logo=logo, result=detections, filename=filename, about=about_page,
